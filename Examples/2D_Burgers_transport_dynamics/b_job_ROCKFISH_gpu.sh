...
resultdir="results/b_Latent-NO/seed=${seed}_n_used=${n_used}"
mkdir -p $resultdir

# Run the papermill command with the passed parameters and save the output notebook in the constructed directory
papermill b_Latent-NO.ipynb "${resultdir}/output_seed=${seed}_n_used=${n_used}.ipynb" -p seed $seed -p n_used $n_used -p save $save > /dev/null 2>&1
