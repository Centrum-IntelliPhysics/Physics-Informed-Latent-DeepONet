...
class Vanilla_NO_model(nn.Module):
    ...
    def forward(self, branch_inputs, trunk_inputs):
        """
        bs    :  Batch size.
        m     :  Number of sensors on each random input field.
        neval :  Number of points at which output field is evaluated for a given random input field sample = nt*nx
...
class Latent_DeepONet(nn.Module):
    ...
    def forward(self, branch_inputs, trunk_inputs):
        ...
        branch inputs shape: (bs x m) 
        trunk inputs shape : (neval_t x 1) # 1 corresponds to t
        
        shapes:  inputs shape         -->      outputs shape
        branch:  (bs x m)             -->      (bs x p)
        trunk:   (neval_t x 1)        -->      (neval_t x p)
        
        reshape p to (d_z x p_)
        
        outputs shape: (bs x neval_t x d_z).
        """
...
class Reconstruction_DeepONet(nn.Module):
    ...
    def forward(self, branch_inputs, trunk_inputs):
        ...
        neval_t :  Number of time points at which latent output field is evaluated for a given random input field sample = nt
        d_z     :  Number of sensors on each latent field. 
        neval_x :  Number of locations at which output field is evaluated at each time point = nx
        q       :  Number of output neurons in both branch and trunk net.   
        
        branch inputs shape: (bs x neval_t x d_z) 
        trunk inputs shape : (neval_x x 1) # 1 corresponds to x
        
        shapes:  inputs shape           -->      outputs shape
        branch:  (bs x neval_t x d_z)   -->      (bs x neval_t x q)
        trunk:   (neval_x x 1)          -->      (neval_x x q)
        
        outputs shape: (bs x neval_t x neval_x).
        """
        
        branch_outputs = self.branch_net(branch_inputs) # (bs, neval_t, q)
        trunk_outputs = self.trunk_net(trunk_inputs) # (neval_x, q)
